...
from .providers import chat_llm
...
def node_classifier(state: "AgentState") -> "AgentState":
    """
    - Construye el caso (ES) y llama a la API externa de clasificación.
    - Opcional: traduce a EN para que 'output' redacte mejor.
    - Guarda 'differentials' y 'case_en'.
    - No emite texto; deja el turno al juez (y luego a output/additional).
    """
    new_state: AgentState = dict(state)
    ...
    # 2) Clasificar vía API externa
    try:
        diffs = classify_case(case_es)
    except Exception as e:
        diffs = []
        # Puedes, si quieres, dejar un aviso mínimo en logs/estado
        new_state["classifier_error"] = str(e)

    # 3) (Opcional) traducir a EN para 'output'
    case_en = _translate_case_to_en(case_es) if case_es else {}

    new_state["differentials"] = diffs
    new_state["case_en"] = case_en or new_state.get("case_en") or {}
    new_state["classifier_done"] = True

    # Importante: NO escribir conversation_response para evitar doble mensaje
    return new_state
